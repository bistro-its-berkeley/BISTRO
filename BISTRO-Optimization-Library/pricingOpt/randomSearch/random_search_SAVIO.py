...
class RandomSearch:
    ...
    def setCordonSearchSpace(self,centroid_list,radii_list,min_radius=None, max_radius=None, fee_range = None):
...
def main():
    ...
    # print(search_object.CONFIG["RESULTS_PATH"])
    # print(input_root)
    ...
    for n in range(int(num_samples)):
        ...
        # prepares runs for simulations with input (calls convert_to_input and passes input directories to SAVIO jobscript compiler):
        runParams = prepRuns_BISTRO_SAVIO(params,search_object.CONFIG["NETWORK_PATH"], search_object.KEEP_FILES, config_path, out_path)
        process_metadata(params,n,headers,out_file,runParams)
        runList.append(str(runParams['folderID']))
    
    print(runList)

def prepRuns_BISTRO_SAVIO(params, network_path,keep_files, config_filename, OUT_PATH):
    """processes inputs and returns corresponding folderID to pass to SAVIO jobscript compiler"""
    
    run_id = uuid.uuid4()
    input_dir = os.path.abspath(f"./submission-inputs/{run_id}")
    ...
    output_dir = os.path.abspath(f"{OUT_PATH}/output/{run_id}")
    if not os.path.exists(output_dir):
        os.system(f'mkdir {output_dir}')

    return {'folderID':run_id,'input_path':input_dir,'output_path':output_dir}


def process_metadata(input_params, sample_n, headers, out_file,runParams):
    this_row ={}
    this_row['sample_num']=sample_n
    p=0
    for h in headers[1:]:
        if h in list(input_params.keys()):
            this_row[h] = input_params[h]
        elif h in list(runParams.keys()):
            this_row[h] = runParams[h]
